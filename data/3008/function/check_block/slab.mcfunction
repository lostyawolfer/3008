...
$execute as @a[gamemode=adventure] if score @s holding.place.$(block).place matches 1.. at @s run fill ~10 ~10 ~10 ~-10 ~-10 ~-10 $(result) replace $(block)[type=double]
...
$execute as @a[gamemode=adventure] if score @s holding.current_block matches $(id) run item replace entity @s container.1 with $(block)[custom_data={game:2}, can_place_on={"blocks": "#3008:everything"}, max_stack_size=1]