...
$execute as @e[type=armor_stand, tag=generating] at @s run place template 3008:plots/long_x/$(plot)_part_1 ~-32 ~-1 ~
$execute as @e[type=armor_stand, tag=generating] at @s run place template 3008:plots/long_x/$(plot)_part_2 ~ ~-1 ~
execute as @e[type=armor_stand, tag=generating] run kill @s

$tellraw @a[scores={logging=1}] ["! log: placed template [3008:$(plot)] at [x:", {"score": {"name": "temp1", "objective": "server"}}, " y:-1", " z:", {"score": {"name": "temp3", "objective": "server"}}, "]"]
