...
execute as @e[type=armor_stand, tag=generating_check] at @s unless block ~ ~-1 ~32 light_gray_concrete run return run function 3008:generation/random_plot/default_plot
...
execute as @e[type=armor_stand, tag=generating_check] at @s unless block ~ ~-1 ~48 light_gray_concrete run return run function 3008:generation/random_plot/default_plot


# target temp-3% and temp-30% for a pillar is 5 when divided by 8
execute if score temp-3% server matches 5 if score temp-30% server matches 5 run return 2

# long_z is for plots 4 chunks long and is placing into future cells
# if temp-30% is 5 then this is a pillar line
# if temp-3% is 5 then this is a pillar column
execute if score temp-3% server matches 2..5 if score temp-30% server matches 5 run return run function 3008:generation/random_plot/default_plot

# $tellraw Lostya "placed plot/long_x/$(plot)"
$function 3008:generation/plot/long_x {plot: $(plot)}
return 0