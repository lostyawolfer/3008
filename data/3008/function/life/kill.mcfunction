execute if score @s health.death_anim matches 17..99 run tellraw @a[scores = {logging = 1..3}] ["! error: ", "something tried to kill ", {"selector": "@s"}, " with reason [$(reason)], but they're already dead"]
execute if score @s health.death_anim matches 1.. run return 1

execute if score @s health.death_anim matches ..-1 run tellraw @a[scores = {logging = 1..3}] ["! error: ", "something tried to kill ", {"selector": "@s"}, " with reason [$(reason)], they're in the middle of respawning"]
execute if score @s health.death_anim matches ..-1 run return 2

execute unless entity @s[gamemode = adventure] run tellraw @a[scores = {logging = 1..3}] ["! error: ", "something tried to kill ", {"selector": "@s"}, " with reason [$(reason)], but they're not in adventure"]
execute unless entity @s[gamemode = adventure] run return 3
...
execute if entity @s[nbt={Inventory:[{id: "minecraft:totem_of_undying"}]}] run tellraw @a[scores = {logging = 1}] ["! log: ", "something tried to kill ", {"selector": "@s"}, " with reason [$(reason)], but they had a totem of undying"]
execute if entity @s[nbt={Inventory:[{id: "minecraft:totem_of_undying"}]}] run scoreboard players set @s health.fall_distance 0
execute if entity @s[nbt={Inventory:[{id: "minecraft:totem_of_undying"}]}] run scoreboard players set @s health.take_damage 0
...
tellraw @a[scores = {logging = 1}] ["! log: ", "something tried to kill ", {"selector": "@s"}, " with reason [$(reason)] and succeeded"]
...
execute if score @s health.last_damage_reason matches 3 run function 3008:life/death_anims/drown
execute if score @s health.last_damage_reason matches 4 run function 3008:life/death_anims/hunger
