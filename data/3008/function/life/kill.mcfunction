$execute if score @s anim.death matches 17..99 run tellraw @a[scores = {logging = 1..3}] ["! error: ", "something tried to kill ", {"selector": "@s"}, " with reason [$(reason)], but they're already dead"]
execute if score @s health.death_anim matches 1.. run return 1

$execute if score @s anim.death matches ..-1 run tellraw @a[scores = {logging = 1..3}] ["! error: ", "something tried to kill ", {"selector": "@s"}, " with reason [$(reason)], they're in the middle of respawning"]
...
stopsound @s
$function 3008:life/death_anims/$(reason)
